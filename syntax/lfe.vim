...
syntax keyword lfeSyntax #%datum #%expression #%top #%variable-reference #%app
syntax keyword lfeSyntax lambda case-lambda let let* letrec
syntax keyword lfeSyntax let-values let*-values let-syntax letrec-syntax
syntax keyword lfeSyntax let-syntaxes letrec-syntaxes letrec-syntaxes+values
syntax keyword lfeSyntax local shared
syntax keyword lfeSyntax if cond and or case define else =>
syntax keyword lfeSyntax begin begin0
syntax keyword lfeSyntax begin-for-syntax
syntax keyword lfeSyntax when unless
syntax keyword lfeSyntax set! set!-values
syntax keyword lfeSyntax for for/list for/vector for/hash for/hasheq for/hasheqv
syntax keyword lfeSyntax for/and for/or for/lists for/first
syntax keyword lfeSyntax for/last for/fold
syntax keyword lfeSyntax for* for*/list for*/vector for*/hash for*/hasheq for*/hasheqv
syntax keyword lfeSyntax for*/and for*/or for*/lists for*/first
syntax keyword lfeSyntax for*/last for*/fold
syntax keyword lfeSyntax for/fold/derived for*/fold/derived
syntax keyword lfeSyntax define-sequence-syntax :do-in do
syntax keyword lfeSyntax with-continuation-mark
syntax keyword lfeSyntax quasiquote unquote unquote-splicing quote-syntax
syntax keyword lfeSyntax #%top-interaction
syntax keyword lfeSyntax define-package open-package package-begin
syntax keyword lfeSyntax define* define*-values define*-syntax define*-syntaxes open*-package
syntax keyword lfeSyntax package? package-exported-identifiers package-original-identifiers
syntax keyword lfeSyntax block #%stratified-body

" 8 Contracts
" 8.2 Function contracts
syntax keyword lfeSyntax -> ->* ->i ->d case-> dynamic->* unconstrained-domain->

" 8.6.1 Nested Contract Boundaries
syntax keyword lfeSyntax with-contract define/contract define-struct/contract
syntax keyword lfeSyntax invariant-assertion current-contract-region

" 9 Pattern Matching
syntax keyword lfeSyntax match match* match/values define/match
syntax keyword lfeSyntax match-lambda match-lambda* match-lambda**
syntax keyword lfeSyntax match-let match-let* match-let-values match-let*-values
syntax keyword lfeSyntax match-letrec match-define match-define-values

" 10.2.3 Handling Exceptions
syntax keyword lfeSyntax with-handlers with-handlers*

" 10.4 Continuations
syntax keyword lfeSyntax let/cc let/ec

" 10.4.1 Additional Control Operators
syntax keyword lfeSyntax % prompt control prompt-at control-at reset shift
syntax keyword lfeSyntax reset-at shift-at prompt0 reset0 control0 shift0
syntax keyword lfeSyntax prompt0-at reset0-at control0-at shift0-at
syntax keyword lfeSyntax set cupto

" 11.3.2 Parameters
syntax keyword lfeSyntax parameterize parameterize*

" 12.5 Writing
syntax keyword lfeSyntax write display displayln print
syntax keyword lfeSyntax fprintf printf eprintf format
syntax keyword lfeSyntax print-pair-curly-braces print-mpair-curly-braces print-unreadable
syntax keyword lfeSyntax print-graph print-struct print-box print-vector-length print-hash-table
syntax keyword lfeSyntax print-boolean-long-form print-reader-abbreviations print-as-expression print-syntax-width
syntax keyword lfeSyntax current-write-relative-directory port-write-handler port-display-handler
syntax keyword lfeSyntax port-print-handler global-port-print-handler

" 13.7 Custodians
syntax keyword lfeSyntax custodian? custodian-memory-accounting-available? custodian-box?
syntax keyword lfeSyntax make-custodian custodian-shutdown-all current-custodian custodian-managed-list
syntax keyword lfeSyntax custodian-require-memory custodian-limit-memory
syntax keyword lfeSyntax make-custodian-box custodian-box-value

" lambda sign
syntax match lfeSyntax /\<[\u03bb]\>/
...
syntax keyword lfeSyntax for/flvector for*/flvector
...
syntax cluster racketTop contains=lfeSyntax,racketFunc,racketDelimiter
